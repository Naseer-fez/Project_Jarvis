...
for root, _, files in os.walk(core_dir):
    ...
    for file in files:
        if file.endswith('.py'):
                    ...
                    if isinstance(node, ast.Import):
                        ...
                        if isinstance(node.func, ast.Attribute):
                            if isinstance(node.func.value, ast.Name):
                                ...
                                if node.func.value.id in ['requests', 'httpx', 'aiohttp', 'urllib', 'aiofiles', 'shutil', 'asyncio']:
            ...
            # Regex for Prompts (multi-line strings containing 'You are')
            try:
                for node in ast.walk(tree):
                    if isinstance(node, ast.Expr) and isinstance(node.value, ast.Constant) and isinstance(node.value.value, str):
                        val_lower = node.value.value.lower()
                        if 'you are ' in val_lower or 'prompt' in val_lower:
                            prompts.append(("docstring_prompt", node.value.value))
            except:
                pass
            ...
            with open(report_path, 'w', encoding='utf-8') as rf:
                ...
                if service_calls:
                    for sc in sorted(service_calls):
                        rf.write(f"- {sc}\n")
                else:
                    rf.write("- None detected\n")
