...
Author  : Naseer-fez
Purpose : Find all uncommitted / unpushed code and copy it to to_commit_pool
...
# Where to create the output pool folder.
# Options:
#   "desktop"   → C:\Users\<you>\Desktop\to_commit_pool             ← DEFAULT
#   "relative"  → same folder as finder.py\to_commit_pool
#   "custom"    → uses POOL_CUSTOM_PATH below
POOL_OUTPUT = "desktop"
POOL_CUSTOM_PATH = r"C:\MyPool\to_commit_pool"  # only used when POOL_OUTPUT = "custom"
...
# Script's own directory — used when SCAN_MODE = "relative"
...
def get_pool_root():
    win_user = get_win_user()
    if POOL_OUTPUT.lower() == "desktop":
        return Path(f"C:/Users/{win_user}/Desktop") / "to_commit_pool"
    elif POOL_OUTPUT.lower() == "relative":
        return SCRIPT_DIR / "to_commit_pool"
    elif POOL_OUTPUT.lower() == "custom":
        return Path(POOL_CUSTOM_PATH)
    else:
        return Path(f"C:/Users/{win_user}/Desktop") / "to_commit_pool"
...
def main():
    ...
    print(f"  {C.MAGENTA}Pool output : {C.BOLD}{POOL_OUTPUT.upper()}{C.RESET}")
    ...
    if total_action == 0:
        ...
        print(f"  {C.YELLOW}{C.BOLD}⚡ Push to_commit_pool/ to GitHub to start the auto-commit workflow.{C.RESET}\n")
