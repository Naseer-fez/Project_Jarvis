...
Covers:
  ...
  6. _handle_web_search() returns a direct failure message when the web tool fails.
"""
...
class TestProcessRoutesToWebSearch:
    ...
    @pytest.mark.asyncio
    async def test_use_internet_live_update_calls_handle_web_search(self) -> None:
        ctrl = _make_minimal_controller()
        ctrl._handle_web_search = AsyncMock(return_value="Live IPL update for you.")
        ...
        with (
            ...
            response = await ctrl.process(
                "use internet and give me the toss update of the current ipl match"
            )

        ctrl._handle_web_search.assert_awaited_once()
        assert "live ipl update" in response.lower()
...
class TestHandleWebSearchFallback:
    @pytest.mark.asyncio
    async def test_formats_structured_results_without_second_llm_pass(self) -> None:
        ctrl = _make_minimal_controller()

        raw = (
            "Search query used: Python 3.13 features\n"
            "Summary: Top results point to the official release notes and related Python resources.\n"
            "Sources:\n"
            "1. Python Docs\n"
        ...
        with (
            ...
            ctrl.llm.chat_async = AsyncMock(return_value="LLM Offline.")

            result = await ctrl._handle_web_search(
                "search for Python 3.13 features", trace_id="test"
            )

        assert "Top results point to the official release notes" in result
        assert "Python Docs (https://docs.python.org)" in result
        ctrl.llm.chat_async.assert_awaited_once()
    ...
    @pytest.mark.asyncio
    async def test_returns_direct_failure_when_search_fails(self) -> None:
        ctrl = _make_minimal_controller()
