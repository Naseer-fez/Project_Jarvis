    ...
    async def process(self, user_input: str, trace_id: str | None = None) -> str:
        ...
        if lowered == "status":
            return _respond(
                ...
                f"Autonomy: {self.autonomy_governor.describe()}"
            )
        if lowered == "help":
            return _respond(
                "Commands: status, help, exit, autonomy [0-3], history, stop, "
                "remember <fact>, what's <query>, open <app>, search <query> in <browser>"
            )
        if lowered == "autonomy":
            return _respond(self.autonomy_governor.describe())
        if lowered.startswith("autonomy "):
            try:
                new_level = int(lowered.split()[-1])
                if self.autonomy_governor.escalate(new_level):
                    return _respond(f"Autonomy changed to {self.autonomy_governor.describe()}")
                return _respond("Cannot escalate above LEVEL_3.")
            except (ValueError, IndexError):
                return _respond(f"Current: {self.autonomy_governor.describe()}. Use 'autonomy 0|1|2|3' to change.")
        if lowered == "history":
            if not self._action_history:
                return _respond("No actions recorded yet this session.")
            lines = []
            for entry in self._action_history[-10:]:
                lines.append(f"  [{entry.get('status', '?')}] {entry.get('action', '?')} — {entry.get('summary', '')}")
            return _respond("Recent actions:\n" + "\n".join(lines))
        if lowered == "stop":
            self.agent_loop.request_interrupt()
            return _respond("Emergency stop requested. Any running task will halt.")
