...
class Dispatcher:
    ...
    async def dispatch(self, action: dict[str, Any]) -> ToolResult:
        ...
        # 1) Desktop actions routed through DesktopActionExecutor contract.
        if tool_name in _DESKTOP_ACTION_MAP:
            risk_score = float(CORE_TOOL_RISK_REGISTRY.get(tool_name, 0.8))
            logger.info("Dispatch desktop action='%s' risk=%.2f rationale='%s'", tool_name, risk_score, rationale)
            return await self._dispatch_desktop_action(tool_name, args, risk_score)

        # 2) Built-in core tool registry.
